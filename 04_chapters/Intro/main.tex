...
\dropcap{A}{utonomous} robots will transform how we live, work, and interact with our environment. Delivery robots could reshape urban logistics, reducing traffic congestion while enabling rapid, sustainable last-mile transport. Agricultural robots offer the potential to monitor crops with unprecedented precision, optimizing yields while minimizing resource consumption. In search-and-rescue operations, autonomous robots could locate survivors in disaster zones far faster than human teams, operating in conditions too dangerous for responders. Environmental monitoring robots could track wildlife populations, measure air quality, and survey ecosystems at scales previously impossible. %~\cite{decroon2022insectinspired}.

For these visions to become reality (and to become forces for good), autonomous robots must meet three essential criteria. They must be \emph{capable}: able to perceive and navigate complex, unpredictable environments, and interact with the world in a useful manner. They must be \emph{safe}: equipped with perception systems robust to changes and failures, and minimal in size and weight to limit physical risk to humans. And they must be \emph{efficient}: able to operate for extended periods without the heavy batteries and powerful processors that now limit their reach and utility.

Current artificial intelligence falls short on all these criteria. State-of-the-art autonomous systems achieve remarkable capabilities, as in the recent demonstration of AI defeating human champions in drone racing~\cite{bahnam2026monorace}, but at substantial computational cost. In the case of flying robots, modern vision systems typically require graphics processing units consuming tens of watts of power, imposing hundreds of grams of payload that directly limit flight time and agility~\cite{nvidia}. Even on the roughly 1~kg quadrotor of \cref{ch:sr}, which needs about 277~W to hover, those watts and grams compound: heavier compute must itself be lifted, demanding a larger battery and motors that must be lifted in turn~\cite{boroujerdian2018mavbench}. On a 30-gram flapping-wing drone flying on about 6~W~\cite{karasek2018tailless}, or on insect-scale platforms with milliwatt sensor suites~\cite{yu2025tinysense}, such hardware lies far beyond the entire flight budget. This creates a fundamental tension: how can we achieve the perceptual capabilities required for safe, useful autonomy while respecting the severe energy constraints of lightweight robotic platforms?

% --- Act 2: Nature's Efficiency Masterclass (~400 words) ---

Nature offers a compelling answer. Biological systems demonstrate that sophisticated perception and agile navigation do not require massive computational resources. For example, honeybees navigate complex environments, avoid obstacles, and find their way home over kilometers of terrain, all with a brain containing fewer than one million neurons~\cite{menzel2001cognitive}. This is not an isolated example: the natural world is full of creatures that perform remarkable computational feats within extreme resource constraints.
...
A third challenge is training data. For event cameras operating at microsecond temporal resolution, obtaining ground truth labels at comparable rates is impractical. Conventional cameras and depth sensors typically operate at rates below 100 Hz, creating a fundamental mismatch between sensor capabilities and supervision signals. This has motivated self-supervised learning approaches that can learn directly from event streams without external ground truth~\cite{gallego2018unifying,zhu2019unsupervised}, but such frameworks are computationally expensive, and challenging to run on embedded platforms.
...
\begin{tcolorbox}[title={\textbf{Main Research Question}}]
    How can robots learn to perceive the world with event-based vision in a way that enables effective behavior in that world?
\end{tcolorbox}
...
Having established learning capability, the next challenge is deployment on actual neuromorphic hardware on board a resource-constrained robot:
...
Inertial measurement units (IMUs) are ubiquitous in flying robots, providing measurements of acceleration and angular velocity considered essential for stable flight. Yet for insect-scale robots, where a sensor weighing tens of milligrams and consuming tens of milliwatts can represent a significant fraction of the total budget~\cite{yu2025tinysense}, eliminating the IMU would be transformative. Biological inspiration supports this possibility: flying insects achieve remarkable agility without dedicated inertial sensors. Success would demonstrate that event cameras can serve as the primary, or potentially only, sensor for autonomous flight.
...
Its central contribution is to show that a flying robot can learn to navigate autonomously with only event-based vision, by combining self-supervised learning, neuromorphic deployment, online adaptation, and sensor minimalism. It makes four contributions, one per chapter:

\begin{enumerate}[leftmargin=*, topsep=4pt, itemsep=1pt, parsep=0pt]
    \item \textbf{Self-supervised learning of event-based optical flow with spiking neural networks} (\cref{ch:neurips}). We introduce modifications to input encoding, loss formulation, and training procedures that enable deep spiking networks to learn optical flow from events self-supervised, matching state-of-the-art conventional networks on standard benchmarks. This establishes that SNNs can learn complex visual tasks, including the dense regression problems central to robotic vision.
    \item \textbf{A fully neuromorphic vision-to-control pipeline on board a flying robot} (\cref{ch:sr}). By dramatically reducing spatial resolution and training a lightweight evolutionary controller, we achieve the first complex autonomous flight with a fully neuromorphic pipeline, deployed on Intel's Loihi neuromorphic processor, operating at 200~Hz while consuming only 7--12~mW for inference.
    \item \textbf{On-board, online self-supervised learning of depth} (\cref{ch:cvpr}). We systematically optimize the contrast maximization framework for time and memory efficiency, so that self-supervised learning can run online, on board during flight, and demonstrate that online learning yields more accurate depth estimates and better obstacle avoidance than offline pre-training alone.
    \item \textbf{Vision-only flight control without an IMU} (\cref{ch:npjr}). A recurrent neural network learns to estimate attitude and rotation rates from events alone, enabling stable flight with vision as the only sensor. This achieves ultimate sensor minimalism, bringing insect-scale flying robots closer to reality.
\end{enumerate}
