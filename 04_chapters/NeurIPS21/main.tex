...
{\it
Realizing the full potential of neuromorphic robot vision requires spiking neural networks (SNNs) that can learn complex visual tasks, yet SNNs have not been successfully applied beyond simple classification problems. This chapter addresses this gap by tackling the self-supervised learning of optical flow estimation from event-based camera inputs with SNNs. We modify the input event representation to encode a much smaller time slice with minimal explicit temporal information, making the network's neuronal dynamics and recurrent connections responsible for integrating information over time. Moreover, we reformulate the self-supervised loss function for event-based optical flow to improve its convexity. For SNNs, we find that parameter initialization and surrogate gradient width play a crucial part in enabling learning with sparse inputs, while the inclusion of adaptive neuronal mechanisms and learnable neuronal parameters can improve performance. The resulting SNNs perform on par with state-of-the-art artificial neural networks trained in a self-supervised manner, demonstrating for the first time that SNNs can solve the dense, real-world visual regression problems essential for robotic perception.
}
...
An event camera consists of a pixel array that responds, in a sparse and asynchronous fashion, to changes in brightness through streams of events \cite{gallego2020eventbased}. For an ideal camera, an event $\boldsymbol{e}_i=(\boldsymbol{x}_i,t_i,p_i)$ of polarity $p_i\in\{+,-\}$ is triggered at pixel $\boldsymbol{x}_i=(x_i,y_i)^T$ and time $t_i$ whenever the brightness change since the last event at that pixel reaches the contrast sensitivity threshold for that polarity.
	...
	\resizebox{\linewidth}{!}{%
		{\renewcommand{\arraystretch}{1.1} 
			\begin{tabular}{lccccccccccc}
				...
				& EPE$\downarrow$ & $\%_{\text{Outlier}}$$\downarrow$&& EPE$\downarrow$ & $\%_{\text{Outlier}}$$\downarrow$&& EPE$\downarrow$ & $\%_{\text{Outlier}}$$\downarrow$&& EPE$\downarrow$ & $\%_{\text{Outlier}}$$\downarrow$\\\midrule
	...
	\resizebox{\linewidth}{!}{%
		{\renewcommand{\arraystretch}{1.1} 
			\begin{tabular}[b]{lccccc}
				...
				\cline{2-3}\cline{5-6}&FWL$\uparrow$&RSAT$\downarrow$&&FWL$\uparrow$&RSAT$\downarrow$ \\\midrule
